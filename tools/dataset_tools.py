...
def generate_fixed_test2idtxt_from_test_negtxt(dataset_path):
    """This function creates two files: `test2id_neg.txt` and `valid2id_neg.txt` from the files
    `test_neg.txt` and `valid_neg.txt` generated by OpenKE when the former files do not exist.
    """
    test_neg = pd.read_csv(dataset_path + '/test_neg.txt', sep='\t', names=['head', 'tail', 'relation', 'label'], skiprows=0)
    valid_neg = pd.read_csv(dataset_path + '/valid_neg.txt', sep='\t', names=['head', 'tail', 'relation', 'label'], skiprows=0)

    test_neg_filter = test_neg.loc[test_neg['label'] == -1]
    valid_neg_filter = valid_neg.loc[valid_neg['label'] == -1]

    with open(dataset_path + '/test2id_neg.txt', 'w') as f:
        f.write('{}\n'.format(len(test_neg_filter)))
        for idx,row in test_neg_filter.iterrows():
            f.write('{} {} {}\n'.format(row['head'], row['tail'], row['relation']))

    with open(dataset_path + '/valid2id_neg.txt', 'w') as f:
        f.write('{}\n'.format(len(valid_neg_filter)))
        for idx,row in valid_neg_filter.iterrows():
            f.write('{} {} {}\n'.format(row['head'], row['tail'], row['relation']))
